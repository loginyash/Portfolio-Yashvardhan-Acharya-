...
export function Footer() {
  ...
  const socialLinks = [
    ...
    { label: 'Behance', href: personal.social.behance },
    ...
    <footer className="w-full border-t border-[rgba(255,255,255,0.04)] bg-[#0A0A0A] py-8 px-6 md:px-12">
      <div className="max-w-[1440px] mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="flex items-center gap-4">
          <svg className="w-5 h-5 text-violet" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="miter">
        ...
        <div className="flex gap-8">
          {socialLinks.map((link, i) => (
            <a
              ...
              className="text-[#F2F2F2] hover:text-violet transition-colors font-subheading text-[11px] uppercase tracking-widest"
            >
              {link.label}
            </a>
          ))}
